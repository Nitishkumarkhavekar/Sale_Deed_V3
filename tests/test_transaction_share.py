...
class TestTheSpecifiedExamples:
    """The cases stated in the instruction, verbatim.

    This rule has been reversed once - a combined split, dividing the deed total
    by sellers plus buyers together, was implemented and then withdrawn. These
    tests pin the per-side rule against the exact figures given, so the two can
    never again be confused for one another.
    """

    def test_ten_thousand_two_sellers_two_buyers(self):
        """₹10,000 ÷ 2 sellers = ₹5,000 each, and ₹10,000 ÷ 2 buyers = ₹5,000
        each. Not ₹2,500."""
        split = _by_side(build_rows([_deed(sellers=2, buyers=2, amount="10000")]))
        assert split["S"] == ["5000", "5000"]
        assert split["B"] == ["5000", "5000"]

    def test_the_combined_split_is_not_used(self):
        """₹10,000 over four persons would be ₹2,500. That is the logic this
        rule replaced and it must not reappear."""
        shares = [r[AMOUNT_COLUMN]
                  for r in build_rows([_deed(sellers=2, buyers=2, amount="10000")])]
        assert "2500" not in shares

    def test_twelve_thousand_three_sellers_two_buyers(self):
        """The sides divide by different counts, so they get different amounts:
        ₹12,000 ÷ 3 = ₹4,000 and ₹12,000 ÷ 2 = ₹6,000."""
        split = _by_side(build_rows([_deed(sellers=3, buyers=2, amount="12000")]))
        assert split["S"] == ["4000"] * 3
        assert split["B"] == ["6000"] * 2

    def test_a_single_seller_receives_the_full_deed_amount(self):
        split = _by_side(build_rows([_deed(sellers=1, buyers=3, amount="9000")]))
        assert split["S"] == ["9000"]

    def test_a_single_buyer_receives_the_full_deed_amount(self):
        split = _by_side(build_rows([_deed(sellers=3, buyers=1, amount="9000")]))
        assert split["B"] == ["9000"]

    @pytest.mark.parametrize("sellers,buyers,amount", [
        (2, 2, "10000"), (3, 2, "12000"), (1, 1, "500"), (1, 5, "9999"),
        (5, 1, "9999"), (4, 7, "1000"), (9, 3, "4473271"), (2, 3, "1"),
    ])
    def test_each_side_always_adds_back_to_the_deed_total(self, sellers, buyers,
                                                          amount):
        """The invariant that makes the rule safe: whichever way the count
        falls, a side's shares reconstitute the deed total exactly."""
        split = _by_side(build_rows([_deed(sellers=sellers, buyers=buyers,
                                           amount=amount)]))
        assert sum(Decimal(s) for s in split["S"]) == Decimal(amount)
        assert sum(Decimal(s) for s in split["B"]) == Decimal(amount)

    def test_two_deeds_do_not_share_a_divisor(self):
        """"Do not mix persons from different deeds" - each transaction
        identity is split against its own parties."""
        ...
        second = DocumentExport(
            ...
            extraction={
                "seller_details": [_person("S9")],
                "buyer_details": [_person("B8"), _person("B9")],
                "property_details": {"sale_consideration": "12000"},
                "document_details": {},
            })
        rows = build_rows([first, second])
        one = _by_side([r for r in rows if r["Transaction Identity"] == "DEED-1"])
        two = _by_side([r for r in rows if r["Transaction Identity"] == "DEED-2"])
        assert one["S"] == ["5000", "5000"] and one["B"] == ["5000", "5000"]
        assert two["S"] == ["12000"] and two["B"] == ["6000", "6000"]

    def test_an_uneven_division_stays_consistent(self):
        """₹1,000 over three sellers cannot divide evenly. The remainder is
        distributed rather than dropped, so the side still totals ₹1,000."""
        split = _by_side(build_rows([_deed(sellers=3, buyers=1, amount="1000")]))
        assert split["S"] == ["333.34", "333.33", "333.33"]
        assert sum(Decimal(s) for s in split["S"]) == Decimal("1000")
        assert split["B"] == ["1000"]
